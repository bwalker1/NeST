...
if __name__ == "__main__":
    ...
    params_list = [neighbor_eps_v, density_v, jaccard_threshold_v, resolution_v]
    ...
    try:
        spatial_genes = int(sys.argv[1])
    except IndexError:
        spatial_genes = 32

    # for spatial_genes in [24, 32, 64, 256, 512]:
    for trial in range(10):
        print(spatial_genes, trial)
        if (
            len(
                glob.glob(
                    f"data/synthetic_hierarchy/results_{spatial_genes}_{trial}.csv"
                )
            )
            > 0
        ):
        ...
        if 1:
            ...
            min_samples = nest.get_min_samples_from_density(
                adata, neighbor_eps_v[0], density_v[0]
            )
            res = nest.compute_gene_hotspots(
                adata_single_gene_hotspots,
                verbose=True,
                log=True,
                eps=neighbor_eps_v[0],
            ...
            nest.coexpression_hotspots(
                adata_ref,
                threshold=jaccard_threshold_v[0],
                min_size=1,
                cutoff=0.3,
                min_genes=min_genes,
            ...
            if 0:
                hmrf = nest.hmrf.HMRFSegmentationModel(
                    adata, regions=5, k=4, label_name="class_hmrf"
                )
        ...
        for param_to_vary in range(len(params_list)):
            for cur_param_idx in range(1, len(params_list[param_to_vary])):
                try:
                    cur_params_list = [
                        params_list[k][cur_param_idx]
                        if k == param_to_vary
                        else params_list[k][0]
                    ...
                    min_samples = nest.get_min_samples_from_density(
                        adata_base, neighbor_eps, density
                    )
                    ...
                    jaccard_matrix, hotspot_lookup = nest.hotspot_jaccard(
                        adata, processes=4, verbose=False, divisions=10
                    )
                    ...
                    try:
                        nest.compute_multi_boundaries(
                            adata, 0.005, 0.00001, verbose=False
                        )
                    except ValueError:
                        continue
