...
def compute_max_jaccard(adata, adata_comp):
    ...
    ch_df_comp = np.array(
        pd.notnull(
            sc.get.obs_df(
                adata_comp,
                keys=[v for v in adata_comp.obs.columns if "hotspots_multi" in v],
            )
        )
    ).astype(np.int_)
    ...
    for idx in range(ch_df_true.shape[1]):
        max_jaccard = max(
            [
                jaccard_score(ch_df_true[:, idx], ch_df_comp[:, idy])
                for idy in range(ch_df_comp.shape[1])
            ]
        )
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        nest.compute_gene_hotspots(
            adata_full,
            verbose=True,
            log=use_modified_otsu_scaling_v[0],
            eps=neighbor_eps_v[0],
            min_samples=min_samples_v[0],
    ...
    nest.plot.multi_hotspots(
        adata_full, color_type=colors, ax=ax, show=False, show_colorbar=True
    )
    fig.savefig(
        os.path.join(image_save_dir, f"subsampling_{frac_genes}.pdf"),
        dpi=300,
        transparent=True,
        bbox_inches="tight",
    )
