...
VISIUM_DATASETS = [
    ...
    "Targeted_Visium_Human_OvarianCancer_Immunology" "V1_Human_Brain_Section_2",
    "V1_Adult_Mouse_Brain_Coronal_Section_1",
...
SPATIAL_LIBD_DATASETS = [
    "Spatial_LIBD_%s" % item
    for item in [
        "151507",
        "151671",
        "151673",
...
def get_data(dataset, dataset_dir=None, normalize=True, **kwargs):
    if dataset in SQUIDPY_DATASETS:
        ...
        if dataset_dir is None:
            raise ValueError(
                "For spatialLIBD data, must set dataset_dir to directory containing the data."
            )
...
def convert_ensembl_names(adata):
    try:
        ...
        raise RuntimeError(
            "Package mygene must be installed to run convert_ensembl_names"
        )
