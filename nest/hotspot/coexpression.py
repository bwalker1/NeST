...
def coexpression_hotspots(
    ...
    if jaccard_matrix is None:
        ...
        jaccard_matrix, hotspot_lookup = hotspot_jaccard(
            adata, processes=processes, verbose=verbose, divisions=divisions
        )

    try:
        multigene_hotspots = hotspot_groups(
            jaccard_matrix,
            hotspot_lookup,
            use_core=use_core,
            core_k=core_k,
        ...
        warnings.warn(
            "No coexpression above threshold was detected. Try again with a lower threshold."
        )
        return
    ...
    multi_arrays = {
        f"hotspots_multi_{idx}": v for idx, (k, v) in enumerate(multi_arrays.items())
    }
    # adata.obs = pd.concat([adata.obs, pd.DataFrame(multi_arrays, index=adata.obs.index)], axis=1)
    adata.obs.drop(
        adata.obs.filter(regex="hotspots_multi").columns.tolist(), axis=1, inplace=True
    )
    tmp_df = adata.obs.merge(
        pd.DataFrame(multi_arrays, index=adata.obs.index),
        how="outer",
        left_index=True,
        right_index=True,
        suffixes=("_old", None),
    ...
    adata.obs = tmp_df
    new_multigene_hotspots = {
        str(idx): v for idx, v in enumerate(new_multigene_hotspots)
    }
    ...
    adata.uns["multi_hotspots_colors"] = {
        k: colors.to_hex(cm1[int(k)]) for k in adata.uns["multi_hotspots"].keys()
    }
...
def multi_hotspot_groupings(adata):
    ...
    multi_hotspots_array = np.nan_to_num(
        np.array(multi_hotspots_df, dtype=np.float32) + 1
    ).astype(np.int_)
...
def hotspot_decomposition(adata: AnnData, neg_weight: float = 1.0, verbose=False):
    ...
    weights_dict = hotspot_decomposition_keys(
        adata,
        all_hotspots,
        multi_hotspots_array,
    ...
    if compute_type_counts:
        types = [
            tuple(np.where(weights)[0])
            for (weights, _) in weights_dict.values()
            if np.sum(weights) > 0
        ]
        for t in types:
...
def hotspot_decomposition_keys(
    adata, keys, multi_hotspots_array, neg_weight=None, verbose=False
):
    weights_dict = {}
    ...
    for hotspot in pbar:
        ...
        hotspot_vector = np.nan_to_num(
            np.array(hotspot_vector, dtype=np.float32)
        ).astype(np.int_)

        weights, score = hotspot_decomposition_sub(
            hotspot_vector, multi_hotspots_array, neg_weight=neg_weight
        )
...
def hotspot_decomposition_sub(hotspot_vector, multi_hotspots_array, neg_weight=None):
    ...
    while True:
        overlap = np.sum(
            np.multiply(hotspot_vector.reshape(-1, 1), multi_hotspots_array), axis=0
        )
        neg_overlap = np.sum(
            np.multiply(1 - hotspot_vector.reshape(-1, 1), multi_hotspots_array), axis=0
        )
        ...
        if score_change[max_ind] > 0:
            ...
            intersects = np.logical_and(
                hotspot_vector, multi_hotspots_array[:, max_ind]
            )
...
def hotspot_jaccard(adata, keys=None, processes=4, verbose=False, divisions=5):
    ...
    if processes is not None:
        for sub_matrix in list_wrapper(
            p.imap_unordered(_multi_wrapper, data_list), total=num_tasks
        ):
            overlap_matrix += sub_matrix
            del sub_matrix
    else:
        for sub_matrix in list_wrapper(
            (_multi_wrapper(data) for data in data_list), total=num_tasks
        ):
            overlap_matrix += sub_matrix
            del sub_matrix
...
def compute_overlap_matrix(iterator_list, num_pairs, num_hotspots, verbose=True):
    ...
    overlap_matrix = coo_matrix(
        (data, (row, col)), (num_hotspots, num_hotspots), dtype=np.float32
    ).tocsr()
    del data, row, col
    if verbose:
        t1 = time.time()
...
def hotspot_groups(
    ...
    if g.number_of_edges() == 0:
        raise RuntimeError(
            "Not enough Jaccard similarity values detected above threshold."
        )
...
def compute_multi_boundaries(adata, alpha_max, alpha_min, ids=None, verbose=False):
    ...
    for idx, hotspot_key in pbar:
        if ids is not None and idx not in ids:
            continue
        boundary = compute_hotspot_boundary(
            adata, hotspot_key, 0, alpha_max=alpha_max, alpha_min=alpha_min
        )
...
def multi_closure(adata):
    ...
    for idx, hotspot_key in enumerate(hotspot_keys):
        ...
        v = pd.Categorical(
            path.contains_points(points, radius=-0.01), categories=(True,)
        )
...
def multiset_index(
    ...
    coex_to_remove: Optional[Iterable] = None,
) -> None:
    """
