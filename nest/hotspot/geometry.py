...
def geometry_scores(adata):
    ...
    for idx, v in adata.uns["multi_hotspots"].items():
        ...
        for key, c in v:
            ...
            if expr.size == 0:
                continue
            arr_unique_expr[sub.astype(np.bool_)] += expr[
                sub.astype(np.bool_)
            ] / np.sum(expr)
    # normalize
    arr = np.log(1 + arr)
    ...
    for cur_ind in inds:
        ...
        for other_ind in inds:
            ...
            try:
                ...
                print(var.shape, var)
        genes_by_fc = [
            v[0] for v in sorted(zip(markers[cur_ind], gene_fc), key=lambda x: -x[1])
        ]
...
def similarity_map(
    adata, idx, adata_ref=None, ax=None, linewidth=0.5, linecolor="black", title=""
):
