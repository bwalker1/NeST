    ...
    if len(list(activity_matrix)) == 0:
        ...
        try:
            ...
            try:
                ...
                um_scale = (
                    adata.uns["spatial"][dataset]["scalefactors"][
                ...
                raise ValueError(
                    "Please set adata.uns['um_scale'] to the length of one micrometer"
                    " in the units of adata.obsm['spatial']."
                ) from None
        ...
        activity_matrix = compute_activity(
            adata,
    ...
    for _, interaction in cols:
        ...
        if z_key is None:
            regions = compute_hotspots(
                adata=adata, input_data=np.where(inds)[0], return_regions=True, **kwargs
            )
        else:
            ...
            for val in np.unique(adata.obs[z_key]):
                ...
                regions_sub = compute_hotspots(
                    adata=adata,
                    input_data=np.where(cur_slice_inds)[0],
                    return_regions=True,
                    **kwargs,
                )
            ...
            regions = pd.Categorical(
                regions, categories=np.arange(1, np.max(regions) + 1)
            )
...
if __name__ == "__main__":
    ...
    n_regions = interaction_hotspots(
        adata,
        eps=neighbor_eps,
