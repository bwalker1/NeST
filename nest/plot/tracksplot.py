...
def tracks_plot(
    ...
    for idx, gene in enumerate(tracksplot_genes):
        ...
        for cat_idx, category in enumerate(categories):
            ...
            expression_values = np.sort(
                obs_cur_cat.loc[:, gene].to_numpy()
            )  # Get the expression_values
            if exp_transform:
    ...
    ax.set_xticklabels(
        category_labels, ha="left", va="top", rotation=label_rotation, fontsize=fontsize
    )
    # groupby_ax.tick_params(bottom=True)
    if type == "hotspot":
        ax.set_xlabel("Hotspot", fontsize=fontsize)
        fig.suptitle(
            " - ".join([v.capitalize() for v in interaction.split("_")]),
            fontsize=fontsize,
            y=0.95,
        )
