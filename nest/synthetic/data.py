...
def create_synthetic_data(
    n_pixels: Tuple[int, Optional[int]],
    n_genes: int,
    spatial_genes: int,
    n_layers: int,
    mean_low: float,
    mean_high: float,
    dropout_active: float,
    type: str = "layer",
    log1p: bool = False,
) -> AnnData:
    """
...
def _get_foreground(pattern: str, xx: ArrayLike, yy: ArrayLike) -> np.ndarray:
    ...
    if pattern == "circle":
        ...
        raise ValueError(
            f"Unkown pattern type: {pattern}. Pattern must be one of ('circle', 'layer')"
        )
...
def compute_sim_vals_2(arr, ref_denom=None):
    ...
    overlap_matrix = (
        coo_matrix((data, (row, col)), (arr.shape[0], arr.shape[0]), dtype=np.float32)
        .tocsr()
        .tocoo()
    )
...
def score_similarity_adata(adata_true, adata_pred):
    ...
    ch_df_true = sc.get.obs_df(
        adata_true, keys=[v for v in adata_true.obs.columns if "hotspots_multi" in v]
    )
    ch_df_pred = sc.get.obs_df(
        adata_pred, keys=[v for v in adata_pred.obs.columns if "hotspots_multi" in v]
    )
...
if __name__ == "__main__":
    ...
    np.random.seed(5)
    if 1:
        adata, layer_matrix = create_synthetic_data(
            n_pixels=64,
            n_genes=256,
            spatial_genes=256,
            n_layers=8,
        ...
        if use_hmrf:
            ...
            hmrf = HMRFSegmentationModel(
                adata=adata_reduced, regions=5, k=4, label_name="class_hmrf"
            )
            hmrf.fit(max_iterations=200, verbose=True, update_labels=True)

            # Compute score
            score = score_fn(
                adata_reduced.obs["regions"], adata_reduced.obs["class_hmrf"]
            )
            print(f"Score is {score}")

            sc.pl.spatial(
                adata_reduced,
                color=("regions", "class_hmrf"),
                frameon=False,
                spot_size=1 / 128,
            )
        ...
        if use_spagcn:
            ...
            score = score_fn(
                adata_reduced.obs["regions"], adata_reduced.obs["class_spagcn"]
            )
            print(f"Score is {score}")
        ...
        if use_nest:
            ...
            min_samples = nest.get_min_samples_from_density(
                adata, neighbor_eps, density=0.2
            )
            hotspot_min_size = 20
            num_hotspot_genes = nest.compute_gene_hotspots(
                adata,
                verbose=True,
                log=True,
                eps=neighbor_eps,
            ...
            # Compute similarity
            ch_df = sc.get.obs_df(
                adata, keys=[v for v in adata.obs.columns if "hotspots_multi" in v]
            )
        ...
        print(
            timeit(lambda: create_synthetic_data(n_pixels=128, n_genes=128), number=100)
            / 100
        )
